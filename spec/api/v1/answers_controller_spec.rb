...
RSpec.describe Api::V1::AnswersController, type: :controller do
  describe 'GET #index' do
    ...
    let(:params) { { access_token: access_token.token, question_id: question.id } }
  ...
  describe 'GET #show' do
    ...
    let(:params) { { access_token: access_token.token, id: answer.id, question_id: question.id } }
  ...
  describe 'POST #create' do
    ...
    let(:access_token) { create(:access_token, resource_owner_id: user.id) }
    let!(:question) { create(:question) }
    let(:params) { { access_token: access_token.token, answer: answer_attributes, question_id: question.id } }
